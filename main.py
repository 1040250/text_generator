...
def generate_text():
    ...
    for s in range(sentences):
        ...
        for w in range(sent_length):
            word, vowel_tracker = '', [1, 1]                        # start new word
            vowel_start = True if 0.5 > np.random.rand() else False # decide if to start with vowel
            word_length = max(1, int(np.random.normal(4.9, 2.7)))       # determine word length: normal(mean, std)
            word_length = word_length + 1 if word_length == 1 and (w == sent_length-1 or word_length_last == 1) else word_length # prevent single-letter word at end of sentence and two in a row
            word_length_last = word_length
            cap = True if cap_prob > np.random.rand() and w > 0 and word_length > 3 else False     # determine if word capitalized (extra letter in addition to word length)
